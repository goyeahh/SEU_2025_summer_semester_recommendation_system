...
class RTCrawler:
    ...
    def __init__(self, config=None):
        ...
        self.network = RTNetwork(self.config)
        self.parser = RTParser(self.config)
        self.data_processor = RTDataProcessor(self.config)
        
        # 设置日志
        self._setup_logging()
        self.logger = logging.getLogger(__name__)
        
        self.logger.info("烂番茄电影爬虫初始化完成")
    ...
    def _setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.FileHandler('rt_crawler.log', encoding='utf-8')
            ]
        )
    ...
    def crawl_movies(self, categories=None, max_movies=None):
        ...
        if categories is None:
            categories = ['most_popular']  # 默认爬取最受欢迎电影
        
        if max_movies is None:
            max_movies = self.config.MAX_MOVIES
        
        self.logger.info(f"开始爬取烂番茄电影数据 - 分类: {categories}, 最大数量: {max_movies}")
    ...
    def _collect_movie_urls(self, categories, max_movies):
        ...
        # 计算需要爬取的页面数量（烂番茄每页通常显示20-30个电影）
        pages_to_crawl = max(1, (movies_per_category + 24) // 25)  # 每页大约25部电影
        ...
        for category in tqdm(categories, desc="解析电影列表页面"):
            ...
            for page in range(1, pages_to_crawl + 1):
                category_url = self.config.get_category_url(category, page=page)
                
                if not category_url:
                    self.logger.warning(f"无效的分类: {category}")
                    continue
                
                # 烂番茄需要JavaScript渲染，直接使用Selenium
                wait_element = '[data-qa="discovery-media-list"], tile-link, .movie_info'
                html = self.network.get_page(category_url, use_selenium=True, wait_element=wait_element)
                ...
                # 处理Cloudflare挑战
                if not self.network.handle_cloudflare_challenge():
                    self.logger.warning(f"Cloudflare挑战处理失败: {category_url}")
                    continue
    ...
    def _crawl_movie_details(self, movie_urls):
        ...
        for url in tqdm(movie_urls, desc="爬取电影详情"):
            try:
                # 烂番茄详情页面需要JavaScript渲染，使用Selenium
                wait_element = '[data-qa="score-panel"], .movie_info, h1'
                html = self.network.get_page(url, use_selenium=True, wait_element=wait_element, timeout=10)
                
                if not html:
                    self.logger.warning(f"获取电影页面失败: {url}")
                    continue
                
                # 简化Cloudflare处理 - 只在必要时检查
                if 'cloudflare' in html.lower() or 'checking your browser' in html.lower():
                    if not self.network.handle_cloudflare_challenge():
                        self.logger.warning(f"Cloudflare挑战处理失败: {url}")
                        continue
    ...
    def test_connection(self):
        ...
        """
        try:
            test_url = "https://www.rottentomatoes.com"
            html = self.network.get_page(test_url, use_selenium=True)
            
            if html and ("rottentomatoes" in html.lower() or "rotten tomatoes" in html.lower()):
                # 处理可能的Cloudflare挑战
                if self.network.handle_cloudflare_challenge():
                    self.logger.info("烂番茄网络连接测试成功")
                    return True
                else:
                    self.logger.error("烂番茄网络连接测试失败（Cloudflare挑战）")
                    return False
            else:
                self.logger.error("烂番茄网络连接测试失败")
                return False
                
        except Exception as e:
            self.logger.error(f"烂番茄网络连接测试异常: {e}")
            return False
